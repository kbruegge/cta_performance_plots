...
def main(gammas_dl3, protons_dl3, electrons_path, output, n_jobs):
    ...
    if electrons_path:
        ...
        electrons['weight'] = mc_production_electrons.reweigh_to_other_spectrum(
            electron_spectrum, electrons.mc_energy.values * u.TeV, t_assumed_obs=t_obs
        )
        electrons['theta'] = (
            calculate_distance_to_point_source(
...
def find_best_detection_significance( theta_square_cuts, prediction_cuts, n_jobs, gammas, background):
    iterator = tqdm(
    ...
    if n_jobs != 1:
        rs = Parallel(n_jobs=n_jobs, verbose=1, prefer='processes', batch_size=40)(
            delayed(calculate_significance)(gammas, background, t, p) for t, p in iterator
        )
    else:
...
def calculate_n_on_n_off(gammas, protons, theta_square_cut, prediction_cut):
    ...
    h, _ = np.histogram(
        protons_gammalike['theta'] ** 2, bins=off_bins, weights=protons_gammalike.weight
    )
    n_off = h.mean()
